...
Технический CSS: `site/assets/css/site.css`. Все 59 страниц подключают его. Это адаптивный каркас с skip-link, видимым focus и `prefers-reduced-motion`.

Главная собирается из `src/data/homepage.json` и `src/templates/homepage.mjs`. Дополнительно подключаются `site/assets/css/home.css` и общий `lead-form.js`. Пока `enabled: false` и endpoint пустой, форма неактивна и не выполняет fetch. Остальные 58 страниц остаются техническими оболочками. Schema.org и `sitemap.xml` не добавляются.
...
Главная получила визуальную систему и подтверждённый контент. Нельзя выдумывать цены, наличие, сроки, урожайность, нормы высева, сорта, документы и преимущества. Endpoint формы, `/privacy/` и Schema подключаются после подтверждения.
