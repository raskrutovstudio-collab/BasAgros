...
function commercial(pages) {
  ...
  return `<section class="mix-section mix-commercial" aria-labelledby="mix-commercial-title"><div class="home-wrap"><div class="mix-commercial-card"><div><p class="home-eyebrow">Поставка</p><h2 id="mix-commercial-title">От выбора смеси до коммерческого предложения</h2><p>Укажите травосмесь или задачу, площадь, объём и населённый пункт. Менеджер подбирает позицию и готовит коммерческое предложение с параметрами поставки.</p>${link('#mix-request', 'Получить коммерческое предложение', 'home-btn home-btn-primary')}</div><div class="mix-commercial-links"><a href="/dostavka-i-oplata/"><strong>Доставка и оплата</strong><span>Параметры поставки и оплаты →</span></a><a href="/kachestvo-i-sertifikaty/"><strong>Качество и документы</strong><span>Характеристики и документы по продукции →</span></a></div></div></div></section>`;
}
