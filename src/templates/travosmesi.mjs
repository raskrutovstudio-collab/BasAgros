...
const PRODUCTS = [
  {
    ...
    text: 'Для формирования кормовой базы хозяйства. Выбор уточняется по назначению посева и необходимому объёму.'
  },
  {
    ...
    text: 'Для задач, где выбор смеси зависит от условий хозяйства. Назначение и необходимый объём уточняются при подборе.'
  },
  {
    ...
    text: 'Для восстановления растительного покрова на рекультивируемых территориях. Площадь и объём учитываются при расчёте.'
  },
  {
    ...
    text: 'Для посева газона и озеленительных задач. Площадь и необходимый объём уточняются при расчёте.'
  },
  {
...
function hero(page, pages) {
  ...
  return `<section class="mix-hero" aria-labelledby="mix-h1"><div class="home-wrap">${breadcrumbs(pages)}<div class="mix-hero-grid"><div class="mix-hero-copy"><p class="home-eyebrow">Каталог BAS Agros</p><h1 id="mix-h1">${escapeHtml(page.h1)}</h1><p class="mix-lead">Семена травосмесей для сельскохозяйственных, кормовых, рекультивационных и озеленительных задач. BAS Agros подбирает травосмесь под задачу хозяйства, площадь и необходимый объём и рассчитывает поставку по Казахстану.</p><div class="home-actions">${link('#mix-products', 'Выбрать травосмесь', 'home-btn home-btn-primary')}${link('#mix-request', 'Получить коммерческое предложение', 'home-btn home-btn-outline', 'data-mix-modal-intent="quote"')}</div><ul class="mix-hero-facts">${facts}</ul></div><div class="mix-hero-media">${heroMedia()}</div></div></div></section>`;
}
...
function criteria() {
  ...
  return `<section class="mix-section mix-criteria" aria-labelledby="mix-criteria-title"><div class="home-wrap mix-two-col"><div><p class="home-eyebrow">Выбор</p><h2 id="mix-criteria-title">Как выбрать травосмесь под задачу</h2><p>Выбор строится на четырёх параметрах: назначение, площадь, объём и место доставки. По ним подбираем позицию и рассчитываем поставку.</p><div class="home-actions mix-criteria-actions">${link('#mix-request', 'Оставить заявку', 'home-btn home-btn-primary', 'data-mix-modal-intent="selection"')}</div></div><ol class="mix-criteria-list">${items.map(([icon, title, text]) => `<li>${mixIcon(icon, 'mix-criteria-icon')}<strong>${title}</strong><span>${text}</span></li>`).join('')}</ol></div></section>`;
}

function compare() {
  const rows = [
    ['Формирование кормовой базы', 'Кормовая', '/catalog/travosmesi/kormovaya/'],
    ['Озимое кормовое направление', 'Рожь + Вика 65/35', '/catalog/travosmesi/rozh-vika-65-35/'],
    ['Восстановление растительного покрова', 'Рекультивационная', '/catalog/travosmesi/rekultivatsionnaya/'],
    ['Газон и озеленение', 'Газонная', '/catalog/travosmesi/gazonnaya/']
  ];
  return `<section class="mix-section mix-compare" aria-labelledby="mix-compare-title"><div class="home-wrap"><div class="mix-section-head"><div><p class="home-eyebrow">Выбор под задачу</p><h2 id="mix-compare-title">Какую травосмесь выбрать под задачу</h2></div><p>Сопоставьте задачу с направлением каталога и перейдите к подходящей позиции.</p></div><div class="mix-compare-table" role="table" aria-label="Выбор травосмеси под задачу"><div class="mix-compare-row mix-compare-header" role="row"><span role="columnheader">Задача</span><span role="columnheader">Травосмесь</span><span role="columnheader">Страница</span></div>${rows.map(([task, product, url]) => `<div class="mix-compare-row" role="row"><strong role="cell">${task}</strong><span role="cell">${product}</span><a role="cell" href="${url}">Перейти →</a></div>`).join('')}</div><p class="mix-compare-note">Состав и точное назначение травосмеси «Универсальная» уточняются при подборе.</p></div></section>`;
}
...
function requestForm() {
  return `<section class="mix-section mix-request" id="mix-request" aria-labelledby="mix-request-title"><div class="home-wrap mix-request-grid"><div><p class="home-eyebrow">Заявка</p><h2 id="mix-request-title">Подобрать травосмесь и рассчитать поставку</h2><p>Укажите назначение, площадь, необходимый объём и место доставки — этих данных достаточно для подбора позиции и расчёта поставки. Название выбранной смеси добавьте в комментарии.</p></div><form class="home-form mix-form" data-lead-form data-form-name="Травосмеси — подбор под задачу"><label for="mix-task">Назначение<select id="mix-task" name="task"><option value="">Выберите задачу</option><option value="Кормовое направление">Кормовое направление</option><option value="Сенокос">Сенокос</option><option value="Пастбище">Пастбище</option><option value="Рекультивация">Рекультивация</option><option value="Газон и озеленение">Газон и озеленение</option><option value="Другое">Другое</option></select></label><label for="mix-area">Площадь посева<input id="mix-area" name="sowing_area" type="text" inputmode="decimal" placeholder="Например, 50 га"></label><label for="mix-volume">Необходимый объём<input id="mix-volume" name="desired_volume" type="text" placeholder="Например, 2 тонны"></label><label for="mix-delivery">Место доставки<input id="mix-delivery" name="delivery_locality" type="text" autocomplete="address-level2" placeholder="Населённый пункт"></label><label for="mix-phone">Телефон<input id="mix-phone" name="phone" type="tel" inputmode="tel" autocomplete="tel" required data-phone-mask maxlength="16" pattern="\\+7 [0-9]{3} [0-9]{3} [0-9]{2} [0-9]{2}" placeholder="+7 XXX XXX XX XX"></label><label class="mix-field-wide" for="mix-message">Травосмесь или комментарий<textarea id="mix-message" name="message" rows="3" placeholder="Например, кормовая травосмесь"></textarea></label><input type="hidden" name="intent" value="travosmesi_selection"><input class="lead-form-honeypot" type="text" name="website" tabindex="-1" autocomplete="off" aria-hidden="true"><p class="mix-consent mix-field-wide">${escapeHtml(CONSENT_TEXT)}</p><button class="home-btn home-btn-primary" type="submit">Подобрать травосмесь</button><div class="home-form-status mix-field-wide" data-form-status aria-live="polite" aria-atomic="true"></div></form></div></section>`;
}

function requestModal() {
  const selectionModal = `<dialog class="home-modal" data-mix-modal="selection" aria-labelledby="mix-selection-modal-title" aria-describedby="mix-selection-modal-description"><div class="home-modal-panel"><button class="home-modal-close" type="button" data-mix-modal-close aria-label="Закрыть форму"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button><p class="home-eyebrow">Подбор под задачу</p><h2 id="mix-selection-modal-title">Подобрать травосмесь</h2><p id="mix-selection-modal-description" class="home-modal-description">Укажите назначение, площадь или объём и место доставки.</p><form class="home-form home-modal-form" data-lead-form data-form-name="Травосмеси — модальное окно — подбор под задачу"><label for="mix-selection-task">Назначение<select id="mix-selection-task" name="task"><option value="">Выберите задачу</option><option value="Кормовое направление">Кормовое направление</option><option value="Сенокос">Сенокос</option><option value="Пастбище">Пастбище</option><option value="Рекультивация">Рекультивация</option><option value="Газон и озеленение">Газон и озеленение</option><option value="Другое">Другое</option></select></label><label for="mix-selection-parameters">Площадь или необходимый объём<input id="mix-selection-parameters" name="message" type="text" placeholder="Например, 50 га или 2 тонны"></label><label for="mix-selection-delivery">Место доставки<input id="mix-selection-delivery" name="delivery_locality" type="text" autocomplete="address-level2" placeholder="Населённый пункт"></label><label for="mix-selection-phone">Телефон<input id="mix-selection-phone" name="phone" type="tel" inputmode="tel" autocomplete="tel" required data-phone-mask maxlength="16" pattern="\\+7 [0-9]{3} [0-9]{3} [0-9]{2} [0-9]{2}" placeholder="+7 XXX XXX XX XX"></label><input type="hidden" name="intent" value="travosmesi_selection"><input class="lead-form-honeypot" type="text" name="website" tabindex="-1" autocomplete="off" aria-hidden="true"><p class="home-modal-note">${escapeHtml(CONSENT_TEXT)}</p><button class="home-btn home-btn-primary" type="submit">Подобрать травосмесь</button><div class="home-form-status" data-form-status aria-live="polite" aria-atomic="true"></div></form></div></dialog>`;
  const quoteModal = `<dialog class="home-modal" data-mix-modal="quote" aria-labelledby="mix-modal-title" aria-describedby="mix-modal-description"><div class="home-modal-panel"><button class="home-modal-close" type="button" data-mix-modal-close aria-label="Закрыть форму"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button><p class="home-eyebrow">Коммерческое предложение</p><h2 id="mix-modal-title">Получить коммерческое предложение</h2><p id="mix-modal-description" class="home-modal-description">Укажите травосмесь или задачу, объём и место доставки.</p><form class="home-form home-modal-form" data-lead-form data-form-name="Травосмеси — модальное окно — коммерческое предложение"><label for="mix-modal-task">Травосмесь или задача<input id="mix-modal-task" name="task" type="text" placeholder="Например, кормовая травосмесь"></label><label for="mix-modal-volume">Необходимый объём<input id="mix-modal-volume" name="desired_volume" type="text" placeholder="Например, 2 тонны"></label><label for="mix-modal-delivery">Место доставки<input id="mix-modal-delivery" name="delivery_locality" type="text" autocomplete="address-level2" placeholder="Населённый пункт"></label><label for="mix-modal-phone">Телефон<input id="mix-modal-phone" name="phone" type="tel" inputmode="tel" autocomplete="tel" required data-phone-mask maxlength="16" pattern="\\+7 [0-9]{3} [0-9]{3} [0-9]{2} [0-9]{2}" placeholder="+7 XXX XXX XX XX"></label><input type="hidden" name="intent" value="travosmesi_quote"><input class="lead-form-honeypot" type="text" name="website" tabindex="-1" autocomplete="off" aria-hidden="true"><p class="home-modal-note">${escapeHtml(CONSENT_TEXT)}</p><button class="home-btn home-btn-primary" type="submit">Получить коммерческое предложение</button><div class="home-form-status" data-form-status aria-live="polite" aria-atomic="true"></div></form></div></dialog>`;
  return `${selectionModal}${quoteModal}<script src="/assets/js/travosmesi-modal.js?v=20260902-2" defer></script>`;
}
...
export function renderTravosmesi(page, pages) {
  ...
  return [hero(page, pages), products(pages), criteria(), compare(), commercial(pages), faq(), requestForm(), requestModal()].join('\n');
}
