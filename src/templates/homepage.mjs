...
function renderHero(page, pages) {
  ...
  return `<section class="home-hero" aria-labelledby="home-h1"><div class="home-wrap home-hero-grid"><div class="home-hero-copy"><p class="home-eyebrow">Семена для сельского хозяйства</p><h1 id="home-h1">${escapeHtml(data.heading)}</h1><p class="home-lead">${escapeHtml(data.text)}</p><div class="home-actions">${link(data.links[0].url, data.links[0].label, 'home-btn home-btn-outline')}${link(data.links[1].url, data.links[1].label, 'home-btn home-btn-primary')}</div></div><div class="home-hero-visual">${mediaSlot('hero', 'home-hero-main')}${mediaSlot('seeds', 'home-hero-detail')}<svg class="home-botanical" viewBox="0 0 140 259" aria-hidden="true"><path d="M76 254V30M76 68C48 50 34 27 40 10M76 100c32-20 44-43 38-62M76 142c-34-20-49-43-43-66M76 184c34-20 49-43 43-66"/></svg></div><ul class="home-hero-categories">${categoryLinks}</ul></div></section>`;
}
